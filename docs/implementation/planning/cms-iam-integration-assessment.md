...
- the remaining physical extraction of governance workflow and governance access implementation is tracked in [SaaS_CMS_Governance_Extraction_Plan.md](./SaaS_CMS_Governance_Extraction_Plan.md)
...
- [CONSTITUTION.md](/Volumes/data/development/IDP/IDP/CONSTITUTION.md)
- [requirements.md](/Volumes/data/development/IDP/IDP/requirements.md)
...
- `/api/v1/cms` is gated through IDP global permission checks using `cms.read` and `cms.manage` in [server.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/server.ts).
- Those global permissions are projected from standalone IAM bearer identity through the same IDP authorization projection path used for other subsystems in [server.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/server.ts).
- A CMS-specific client role `cms-admin` exists in IAM seed data in [iamFoundation.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/iamFoundation.ts).
- CMS API tokens are implemented for machine-to-machine access in [cmsAccessRuntime.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/cmsAccessRuntime.ts).
- CMS role families and CMS role assignments exist as standalone CMS domain objects in [cmsAccessRuntime.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/cmsAccessRuntime.ts).
...
The strongest remaining defect is in [cmsAccessRuntime.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/cmsAccessRuntime.ts) and the server composition layer in [server.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/server.ts), even though the public route namespace has been neutralized.
...
[cmsAccessRuntime.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/cmsAccessRuntime.ts) stores:
...
- a CMS validation realm in [iamFoundation.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/iamFoundation.ts)
- `cms-admin`
- `content-editor`
...
I did not find a projection path that turns the seeded IAM `content-editor` role into effective CMS interactive permissions. The UI in [HeadlessCms.tsx](/Volumes/data/development/IDP/IDP/apps/enterprise-ui/src/pages/HeadlessCms.tsx) also keys mutation behavior off `hasPermission('cms.manage')`, which is the global platform permission, not a CMS-native editor or reviewer capability.
...
[HeadlessCms.tsx](/Volumes/data/development/IDP/IDP/apps/enterprise-ui/src/pages/HeadlessCms.tsx) determines management capability with `hasPermission('cms.manage')`.
...
The current public delivery endpoints in [server.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/server.ts) pass mostly query-driven inputs such as:
...
The delivery runtime in [cmsDeliveryRuntime.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/cmsDeliveryRuntime.ts) resolves locale and audience, and personalization can apply audience branches, but this is not yet the same as full canonical IAM- and entitlement-aware content resolution.
...
Create a user-side evaluator in [cmsAccessRuntime.ts](/Volumes/data/development/IDP/IDP/apps/api-server/src/platform/cmsAccessRuntime.ts) that can resolve:
...
- the remaining workflow and access implementation still needs to move into `saas-cms`, as defined in [SaaS_CMS_Governance_Extraction_Plan.md](./SaaS_CMS_Governance_Extraction_Plan.md)
