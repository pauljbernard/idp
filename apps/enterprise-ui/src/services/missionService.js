...
export class MissionService {
    constructor() {
        Object.defineProperty(this, "missions", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    async createMission(missionData) {
        const mission = this.syncMissionAliases({
            ...missionData,
            id: this.generateId(),
     ...
     */
    async createMissionFromTemplate(templateId, overrides) {
     ...
     */
    async searchMissions(filters, sort, limit, offset) {
        let missions = Array.from(this.missions.values());
        // Apply filters
        if (filters) {
            missions = missions.filter(mission => {
                if (filters.status && !filters.status.includes(mission.status))
                    return false;
                if (filters.type && !filters.type.includes(mission.type))
                    return false;
                if (filters.priority && !filters.priority.includes(mission.priority))
                    return false;
                ...
                if (requestedSpecialist && mission.assigned_pilot !== requestedSpecialist && mission.assigned_specialist !== requestedSpecialist)
                    return false;
                const requestedResource = filters.resource ?? filters.aircraft;
                if (requestedResource && mission.assigned_aircraft !== requestedResource && mission.assigned_resource !== requestedResource)
                    return false;
                if (filters.location_radius) {
                    const distance = this.calculateDistance(mission.location.lat, mission.location.lon, filters.location_radius.lat, filters.location_radius.lon);
                    if (distance > filters.location_radius.radius_km)
                        return false;
                }
                if (filters.tags && !filters.tags.some(tag => mission.tags.includes(tag)))
                    return false;
     ...
     */
    async getUpcomingMissions(limit = 10) {
        ...
        }, { field: 'scheduled_start', direction: 'asc' }, limit);
        return missions;
    }
    /**
     * Add note to mission
     */
    async addMissionNote(missionId, note) {
        const mission = this.missions.get(missionId);
        if (!mission) {
            throw new Error(`Mission not found: ${missionId}`);
        }
     ...
     */
    async shareMission(missionId, userIds) {
        const mission = this.missions.get(missionId);
        if (!mission) {
            throw new Error(`Mission not found: ${missionId}`);
        }
     ...
     * Duplicate mission
     */
    async duplicateMission(missionId, overrides) {
        ...
        delete duplicatedMissionData.id;
        delete duplicatedMissionData.created_at;
        delete duplicatedMissionData.updated_at;
        delete duplicatedMissionData.notes;
        delete duplicatedMissionData.attachments;
    ...
    async createRecurringInstances(parentMission) {
        if (!parentMission.recurrence_pattern)
            return;
