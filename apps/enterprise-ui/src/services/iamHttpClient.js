...
import { clearAuthenticatedSession, clearIamBrowserAuth, getActiveTenantId, getCurrentIamAccessToken, getCurrentIamAuthClientId, getCurrentIamAuthRealmId, getCurrentIamRealmId, getCurrentIamRefreshToken, getCurrentIamSessionId, getCurrentSessionId, getCurrentUserId, setCurrentIamAccessToken, setCurrentIamRefreshToken, shouldRefreshIamBrowserAccessToken, } from './iamClientState';
...
let currentIamRefreshPromise = null;
async function refreshCurrentIamBrowserAccessToken() {
    ...
    currentIamRefreshPromise = (async () => {
        try {
            ...
            const response = await publicApi.post(`/iam/realms/${realmId}/protocol/openid-connect/token`, body, {
                headers: {
